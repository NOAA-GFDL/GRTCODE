...
/* Ported from Gamache's Fortran TIPS_2011 by Garrett Wright for internal use at GFDL */

/*    Program TIPS_2011 written by R.R. Gamache
...
#define NT 119 /* number tempurate samples from TIPS*/
/* Tdat, later called "A" in function, consistent with original TIPS code */
const REAL_t Tdat[NT] = { 60. ,  85. , 110. , 135. , 160. , 185. , 210. , 235. , 260. ,
                          285. , 310. , 335. , 360. , 385. , 410. , 435. , 460. , 485. ,
                          510. , 535. , 560. , 585. , 610. , 635. , 660. , 685. , 710. ,
                          735. , 760. , 785. , 810. , 835. , 860. , 885. , 910. , 935. ,
                          960. , 985. , 1010. , 1035. , 1060. , 1085. , 1110. , 1135. ,
                          1160. , 1185. , 1210. , 1235. , 1260. , 1285. , 1310. , 1335. ,
                          1360. , 1385. , 1410. , 1435. , 1460. , 1485. , 1510. , 1535. ,
                          1560. , 1585. , 1610. , 1635. , 1660. , 1685. , 1710. , 1735. ,
                          1760. , 1785. , 1810. , 1835. , 1860. , 1885. , 1910. , 1935. ,
                          1960. , 1985. , 2010. , 2035. , 2060. , 2085. , 2110. , 2135. ,
                          2160. , 2185. , 2210. , 2235. , 2260. , 2285. , 2310. , 2335. ,
                          2360. , 2385. , 2410. , 2435. , 2460. , 2485. , 2510. , 2535. ,
                          2560. , 2585. , 2610. , 2635. , 2660. , 2685. , 2710. , 2735. ,
                          2760. , 2785. , 2810. , 2835. , 2860. , 2885. , 2910. , 2935. ,
                          2960. , 2985. , 3010. };
#ifdef __CUDA_ARCH__
__constant__ REAL_t A[NT];
#else
const REAL_t* const A = Tdat;
#endif

#ifdef __NVCC__
__host__ int initTIPS_d(){
  HANDLE_ERROR( cudaMemcpyToSymbol(A, Tdat, sizeof(REAL_t)*NT) );
  return 0;
}
#endif
...
__host__ __device__
#endif
REAL_t AtoB(const REAL_t aa,
            const REAL_t* const B,
            const REAL_t npt){

  const REAL_t eps=0.0001; /*  dividing by zero is bad */
  REAL_t bb=-1; /* result, -1 is bad */
  /* iters */
  int i;
  int j;
  /* tmps */
  ...
  for( i=2; i<=npt; ++i){
    if (A[i-1]>=aa){
      if( i < 3 || i == npt ){
        /* LaGrange Three Point Interp */

        j = i-1; /* corect for Zero based arrays */
        if ( i < 3){
          j = 3-1; /* corect for Zero based arrays */
        }
        if (i == npt ){
          j = (npt-1); /* corect for Zero based arrays */
        }
      ...
      } /* endIF(I.LT.3 .OR. I.EQ.npt) */
      break; /* goto 100 */
    } /* endIF(A(I).GE.aa) */
  } /* end loop label 50 */
  /*  label 100 */
#ifdef LABEL_100
  printf("F1, F2, F3, H1, H2, H3 = %f, %f, %f, %f, %f, %f\n", B[j-2], B[j-1], B[j], A[j-2], A[j-1], A[j] );
  printf("A0, A1, A2, bb = %f, %f, %f, %f\n", A0, A1, A2, bb );
#endif
...
__host__ __device__
#endif
void QT_H2O ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{

  const REAL_t xgj[6] = {  1. , 1. , 6. , 6. , 6. , 36 } ;
  /*       H2O */
  const REAL_t QofT[6][NT] = {
    /*         --       161 */
    { 0.16824E+02, 0.27771E+02, 0.40408E+02,
      ...
      0.16147E+05 } ,
    /*     --       181 */
    { 0.15960E+02, 0.26999E+02, 0.39743E+02,
      ...
      0.16225E+05 } ,
    /*         --       171 */
    { 0.95371E+02, 0.16134E+03, 0.23750E+03,
      ...
      0.96743E+05 } ,
    /*         --       162 */
    { 0.75792E+02, 0.12986E+03, 0.19244E+03,
      ...
      0.99064E+05 } ,
    /*         --       182 */
    { 0.82770E+02, 0.13749E+03, 0.20083E+03,
      ...
      0.10067E+06 } ,
    /*         --       172 */
    { 0.49379E+03, 0.82021E+03, 0.11980E+04,
  ...
  *gsi = xgj[iso];
  ...
  if(T<70. || T>3000.){
    *Qt = -1. ;
    printf("%f OUT OF TEMPERATURE RANGE",T);
  }
  else{
    *Qt = AtoB(T,Q,NT);
  }
...
__host__ __device__
#endif
void QT_CO2 ( const REAL_t T,   /*  temperature in K */
              int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{

  /* CO2 is different because iso w/ AFGL 838 is HITRAN index 0, but TIP index 10 */
  /* if we assume QT has sent us a decremented (ie zero based) index.. */
  /* -1 should resolve to 10th item (index 9) */
  /* 11 appears to be unused at this time, because HITRAN format is 0-9 */
  if (iso == -1){
    iso = 9;
  }

  const REAL_t xgj[11] = {  1. , 2. , 1. , 6. , 2. , 12. , 1. , 6. , 1. , 2. , 12. } ;
  /* CO2 */
  const REAL_t QofT[11][NT] = {
    /*         --       626 */
    { 0.53642E+02, 0.75947E+02, 0.98292E+02,
      ...
      0.11351E+06 } ,
    /*         --       636 */
    { 0.10728E+03, 0.15189E+03, 0.19659E+03,
      ...
      0.24436E+06 } ,
    /*         --       628 */
    { 0.11368E+03, 0.16096E+03, 0.20833E+03,
      ...
      0.24750E+06 } ,
    /*         --       627 */
    { 0.66338E+03, 0.93923E+03, 0.12156E+04,
      ...
      0.14255E+07 } ,
    /*         --       638 */
    { 0.22737E+03, 0.32194E+03, 0.41671E+03,
      ...
      0.53506E+06 } ,
    /*         --       637 */
    { 0.13267E+04, 0.18785E+04, 0.24314E+04,
      ...
      0.30747E+07 } ,
    /*         --       828 */
    { 0.60334E+02, 0.85430E+02, 0.11058E+03,
      ...
      0.13497E+06 } ,
    /*         --       728 */
    { 0.70354E+03, 0.99615E+03, 0.12893E+04,
      ...
      0.15539E+07 } ,
    /*         --       727 */
    { 0.20518E+04, 0.29051E+04, 0.37601E+04,
      ...
      0.44739E+07 } ,
    /*         --       838 */
    { 0.12066E+03, 0.17085E+03, 0.22116E+03,
      ...
      0.29064E+06 } ,
    /*         --       837 */
    { 0.14071E+04, 0.19923E+04, 0.25789E+04,
...
__host__ __device__
#endif
void QT_O3 ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[18] ={ 1. , 1. , 1. , 6. , 6. , 1. , 1. , 6. , 6. , 6. , 36. , 1. , 1. , 6. , 6. , 36. , 1. , 6.};
  /*        O3 */
  const REAL_t QofT[18][NT] = {
    /*         --       666 */
    { 0.30333E+03, 0.51126E+03, 0.75274E+03,
      ...
      0.24883E+07 } ,
    /*         --       668 */
    { 0.64763E+03, 0.10916E+04, 0.16073E+04,
      ...
      0.55261E+07 } ,
    /*         --       686 */
    { 0.31656E+03, 0.53355E+03, 0.78557E+03,
      ...
      0.27432E+07 } ,
    /*         --       667 */
    { 0.37657E+04, 0.63472E+04, 0.93454E+04,
      ...
      0.31531E+08 } ,
    /*         --       676 */
    { 0.18608E+04, 0.31363E+04, 0.46177E+04,
      ...
      0.15706E+08 } ,
    /*         --       886 */
    { 0.67639E+03, 0.11401E+04, 0.16787E+04,
      ...
      0.61050E+07 } ,
    /*         --       868 */
    { 0.34615E+03, 0.58348E+03, 0.85915E+03,
      ...
      0.30749E+07 } ,
    /*         --       678 */
    { 0.39745E+04, 0.66993E+04, 0.98642E+04,
      ...
      0.34936E+08 } ,
    /*         --       768 */
    { 0.40228E+04, 0.67808E+04, 0.99842E+04,
      ...
      0.35053E+08 } ,
    /*         --       786 */
    { 0.39315E+04, 0.66267E+04, 0.97569E+04,
      ...
      0.34829E+08 } ,
    /*         --       776 */
    { 0.23106E+05, 0.38945E+05, 0.57342E+05,
      ...
      0.19912E+09 } ,
    /*         --       767 */
    { 0.11692E+05, 0.19707E+05, 0.29017E+05,
      ...
      0.99896E+08 } ,
    /*         --       888 */
    { 0.36175E+03, 0.60978E+03, 0.89790E+03,
      ...
      0.34040E+07 } ,
    /*         --       887 */
    { 0.42000E+04, 0.70796E+04, 0.10424E+05,
      ...
      0.38816E+08 } ,
    /*         --       878 */
    { 0.21250E+04, 0.35820E+04, 0.52744E+04,
      ...
      0.19481E+08 } ,
    /*         --       778 */
    { 0.24692E+05, 0.41621E+05, 0.61284E+05,
      ...
      0.22183E+09 } ,
    /*         --       787 */
    { 0.12211E+05, 0.20582E+05, 0.30305E+05,
      ...
      0.11059E+09 } ,
    /*         --       777 */
    { 0.71750E+05, 0.12094E+06, 0.17807E+06,
...
__host__ __device__
#endif
void QT_N2O ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[5] = {9. , 6. , 6. , 9. , 54.  } ;
  /* N2O */
  const REAL_t QofT[5][NT] = {
    /*         --       446 */
    { 0.89943E+03, 0.12734E+04, 0.16489E+04,
      ...
      0.25735E+07 } ,
    /*         --       456 */
    { 0.59966E+03, 0.84903E+03, 0.10995E+04,
      ...
      0.18109E+07 } ,
    /*         --       546 */
    { 0.62051E+03, 0.87856E+03, 0.11377E+04,
      ...
      0.18194E+07 } ,
    /*         --       448 */
    { 0.95253E+03, 0.13487E+04, 0.17465E+04,
      ...
      0.28284E+07 } ,
    /*         --       447 */
    { 0.55598E+04, 0.78718E+04, 0.10193E+05,
...
__host__ __device__
#endif
void QT_CO ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[6] = {1. , 2. , 1. , 6. , 2. , 12. } ;
  /*        CO */
  const REAL_t QofT[6][NT] = {
    /*         --        26 */
    { 0.21948E+02, 0.30961E+02, 0.39980E+02,
      ...
      0.17536E+04 } ,
    /*         --        36 */
    { 0.45888E+02, 0.64745E+02, 0.83615E+02,
      ...
      0.37163E+04 } ,
    /*         --        28 */
    { 0.23030E+02, 0.32495E+02, 0.41966E+02,
      ...
      0.18673E+04 } ,
    /*         --        27 */
    { 0.13505E+03, 0.19054E+03, 0.24606E+03,
      ...
      0.10874E+05 } ,
    /*         --        38 */
    { 0.48264E+02, 0.68112E+02, 0.87974E+02,
      ...
      0.39699E+04 } ,
    /*         --        37 */
    { 0.28271E+03, 0.39894E+03, 0.51524E+03,
...
__host__ __device__
#endif
void QT_CH4 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[4] = {1. , 2. , 3. , 6. } ;
  /* CH4 */
  const REAL_t QofT[4][NT] = {
    /*         --       211 */
    { 0.54800E+02, 0.91500E+02, 0.13410E+03,
      ...
      0.42856E+07 } ,
    /*         --       311 */
    { 0.10958E+03, 0.18304E+03, 0.26818E+03,
      ...
      0.42673E+07 } ,
    /*         --       212 */
    { 0.44079E+03, 0.73786E+03, 0.10822E+04,
      ...
      0.25369E+08 } ,
    /*         --       312 */
    { 0.88231E+03, 0.14770E+04, 0.21661E+04,
...
__host__ __device__
#endif
void QT_O2 ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[3] = {1. , 1. , 6. } ;
  /*        O2 */
  const REAL_t QofT[3][NT] = {
    /*         --        66 */
    { 0.44334E+02, 0.62460E+02, 0.80596E+02,
      ...
      0.44132E+04 } ,
    /*         --        68 */
    { 0.89206E+02, 0.12759E+03, 0.16600E+03,
      ...
      0.92988E+04 } ,
    /*         --        67 */
    { 0.52071E+03, 0.74484E+03, 0.96908E+03,
...
__host__ __device__
#endif
void QT_NO ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[3] = {3. , 2. , 3. } ;
  /*        NO */
  const REAL_t QofT[3][NT] = {
    /*         --        46 */
    { 0.15840E+03, 0.23971E+03, 0.33080E+03,
      ...
      0.24517E+05 } ,
    /*         --        56 */
    { 0.10942E+03, 0.16560E+03, 0.22856E+03,
      ...
      0.17131E+05 } ,
    /*         --        48 */
    { 0.16695E+03, 0.25269E+03, 0.34876E+03,
...
__host__ __device__
#endif
void QT_SO2 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[2] = {1. , 1. } ;
  /* SO2 */
  const REAL_t QofT[2][NT] = {
    /*         --       626 */
    { 0.52899E+03, 0.89171E+03, 0.13139E+04,
      ...
      0.43479E+07 } ,
    /*         --       646 */
    { 0.53140E+03, 0.89578E+03, 0.13199E+04,
...
__host__ __device__
#endif
void QT_NO2 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[1] = {3. } ;
  /* NO2 */
  const REAL_t QofT[1][NT] = {
    /*         --       646 */
    { 0.12046E+04, 0.20297E+04, 0.29875E+04,
...
__host__ __device__
#endif
void QT_NH3 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[2] = {3. , 2. } ;
  /* NH3 */
  const REAL_t QofT[2][NT] = {
    /*         --      4111 */
    { 0.16013E+03, 0.26692E+03, 0.39067E+03,
      ...
      0.10207E+07 } ,
    /*         --      5111 */
    { 0.10697E+03, 0.17832E+03, 0.26100E+03,
...
__host__ __device__
#endif
void QT_HNO3 ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[1] = {6. } ;
  /*      HNO3 */
  const REAL_t QofT[1][NT] = {
    /*         --       146 */
    { 0.15010E+05, 0.25316E+05, 0.37374E+05,
...
__host__ __device__
#endif
void QT_OH ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[3] = {2. , 2. , 3. } ;
  /*        OH */
  const REAL_t QofT[3][NT] = {
    /*         --        61 */
    { 0.20066E+02, 0.24774E+02, 0.30309E+02,
      ...
      0.11138E+04 } ,
    /*         --        81 */
    { 0.20124E+02, 0.24876E+02, 0.30457E+02,
      ...
      0.12837E+04 } ,
    /*         --        62 */
    { 0.41032E+02, 0.54704E+02, 0.70201E+02,
...
__host__ __device__
#endif
void QT_HF ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[1] = {4. } ;
  /*        HF */
  const REAL_t QofT[1][NT] = {
    /*         --        19 */
    { 0.95958E+01, 0.12933E+02, 0.16295E+02,
...
__host__ __device__
#endif
void QT_HCl ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[2] = {8. , 8. } ;
  /* HCl */
  const REAL_t QofT[2][NT] = {
    /*         --        15 */
    { 0.34775E+02, 0.48060E+02, 0.61370E+02,
      ...
      0.22295E+04 } ,
    /*         --        17 */
    { 0.34823E+02, 0.48128E+02, 0.61458E+02,
...
__host__ __device__
#endif
void QT_HBr ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[2][NT] = {
    /*         --        19 */
    { 0.42744E+02, 0.59373E+02, 0.76023E+02,
      ...
      0.29693E+04 } ,
    /*         --        11 */
    { 0.42756E+02, 0.59390E+02, 0.76045E+02,
...
__host__ __device__
#endif
void QT_HI ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[1] = {12. } ;
  /*        HI */
  const REAL_t QofT[1][NT] = {
    /*         --        17 */
    { 0.82031E+02, 0.11447E+03, 0.14694E+03,
...
__host__ __device__
#endif
void QT_ClO ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[2] = {4. , 4. } ;
  /* ClO */
  const REAL_t QofT[2][NT] = {
    /*         --        56 */
    { 0.53847E+03, 0.76580E+03, 0.10017E+04,
      ...
      0.15797E+06 } ,
    /*         --        76 */
    { 0.54775E+03, 0.77899E+03, 0.10189E+04,
...
__host__ __device__
#endif
void QT_OCS ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[5] = {1. , 1. , 2. , 4. , 1. } ;
  /* OCS */
  const REAL_t QofT[5][NT] = {
    /*         --       622 */
    { 0.20609E+03, 0.29199E+03, 0.37861E+03,
      ...
      0.10431E+07 } ,
    /*         --       624 */
    { 0.21125E+03, 0.29930E+03, 0.38809E+03,
      ...
      0.10837E+07 } ,
    /*         --       632 */
    { 0.41351E+03, 0.58591E+03, 0.76004E+03,
      ...
      0.22507E+07 } ,
    /*         --       623 */
    { 0.83485E+03, 0.11828E+04, 0.15337E+04,
      ...
      0.42546E+07 } ,
    /*         --       822 */
    { 0.21967E+03, 0.31126E+03, 0.40370E+03,
...
__host__ __device__
#endif
void QT_H2CO ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[3] = {1. , 2. , 1. } ;
  /*      H2CO */
  const REAL_t QofT[3][NT] = {
    /*         --       126 */
    { 0.25934E+03, 0.43623E+03, 0.64143E+03,
      ...
      0.30926E+07 } ,
    /*         --       136 */
    { 0.53173E+03, 0.89447E+03, 0.13153E+04,
      ...
      0.63421E+07 } ,
    /*         --       128 */
    { 0.27198E+03, 0.45755E+03, 0.67282E+03,
...
__host__ __device__
#endif
void QT_HOCl ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[2][NT] = {
    /*         --       165 */
    { 0.17041E+04, 0.28708E+04, 0.42250E+04,
      ...
      0.61755E+07 } ,
    /*         --       167 */
    { 0.17342E+04, 0.29215E+04, 0.42998E+04,
...
__host__ __device__
#endif
void QT_N2 ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[1] = {1. } ;
  /*        N2 */
  const REAL_t QofT[1][NT] = {
    /*         --        44 */
    { 0.95487E+02, 0.13466E+03, 0.17386E+03,
...
__host__ __device__
#endif
void QT_HCN ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[3] = {6. , 12. , 4. } ;
  /* HCN */
  const REAL_t QofT[3][NT] = {
    /*         --       124 */
    { 0.17143E+03, 0.24209E+03, 0.31285E+03,
      ...
      0.20986E+06 } ,
    /*         --       134 */
    { 0.35186E+03, 0.49693E+03, 0.64221E+03,
      ...
      0.43631E+06 } ,
    /*         --       125 */
    { 0.11863E+03, 0.16755E+03, 0.21653E+03,
...
__host__ __device__
#endif
void QT_CH3Cl ( const REAL_t T,   /*  temperature in K */
                const int8_t iso, /*  isotope code (HITRANINDEX) */
                float* const gsi,       /*  state independent nuclear degeneracyfactor */
                REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[2][NT] = {
    /*         --       215 */
    { 0.50529E+04, 0.85123E+04, 0.12528E+05,
      ...
      0.78573E+09 } ,
    /*         --       217 */
    { 0.51327E+04, 0.86469E+04, 0.12726E+05,
...
__host__ __device__
#endif
void QT_H2O2 ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --      1661 */
    { 0.62392E+03, 0.10958E+04, 0.16692E+04,
...
__host__ __device__
#endif
void QT_C2H2 ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[3] = {1. , 8. , 6. } ;
  /*      C2H2 */
  const REAL_t QofT[3][NT] = {
    /*         --      1221 */
    { 0.71617E+02, 0.10121E+03, 0.13092E+03,
      ...
      0.17040E+07 } ,
    /*         --      1231 */
    { 0.28647E+03, 0.40486E+03, 0.52369E+03,
      ...
      0.68162E+07 } ,
    /*         --      1222 */
    { 0.24843E+03, 0.35373E+03, 0.45997E+03,
...
__host__ __device__
#endif
void QT_C2H6 ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[2] = {1. , 2. } ;
  /*      C2H6 */
  const REAL_t QofT[2][NT] = {
    /*         --      1221 */
    { 0.47267E+04, 0.80011E+04, 0.11928E+05,
      ...
      0.58542E+12 } ,
    /*         --      1231 */
    { 0.24128E+04, 0.40845E+04, 0.60896E+04,
...
__host__ __device__
#endif
void QT_PH3 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[1] = {2. } ;
  /* PH3 */
  const REAL_t QofT[1][NT] = {
    /*         --      1111 */
    { 0.29652E+03, 0.49643E+03, 0.72810E+03,
...
__host__ __device__
#endif
void QT_COF2 ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --       269 */
    { 0.54999E+04, 0.92749E+04, 0.13668E+05,
...
__host__ __device__
#endif
void QT_SF6 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --        29 */
    { 0.46373E+05, 0.78844E+05, 0.11939E+06,
...
__host__ __device__
#endif
void QT_H2S ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[3] = {1. , 1. , 4. } ;
  /* H2S */
  const REAL_t QofT[3][NT] = {
    /*         --       121 */
    { 0.47192E+02, 0.78671E+02, 0.11510E+03,
      ...
      0.73318E+05 } ,
    /*         --       141 */
    { 0.47310E+02, 0.78869E+02, 0.11539E+03,
      ...
      0.73505E+05 } ,
    /*         --       131 */
    { 0.18901E+03, 0.31509E+03, 0.46102E+03,
...
__host__ __device__
#endif
void QT_HCOOH ( const REAL_t T,   /*  temperature in K */
                const int8_t iso, /*  isotope code (HITRANINDEX) */
                float* const gsi,       /*  state independent nuclear degeneracyfactor */
                REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --       126 */
    { 0.31899E+04, 0.53773E+04, 0.79205E+04,
...
__host__ __device__
#endif
void QT_HO2 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --       166 */
    { 0.39277E+03, 0.66062E+03, 0.97123E+03,
...
__host__ __device__
#endif
void QT_O ( const REAL_t T,   /*  temperature in K */
            const int8_t iso, /*  isotope code (HITRANINDEX) */
            float* const gsi,       /*  state independent nuclear degeneracyfactor */
            REAL_t* const Qt)       /*  Total Internal Partition Function */
...
__host__ __device__
#endif
void QT_ClONO2 ( const REAL_t T,   /*  temperature in K */
                 const int8_t iso, /*  isotope code (HITRANINDEX) */
                 float* const gsi,       /*  state independent nuclear degeneracyfactor */
                 REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[2] = {12. , 12. } ;
  /*    ClONO2 */
  const REAL_t QofT[2][NT] = {
    /*         --      5646 */
    { 0.11444E+06, 0.21121E+06, 0.34858E+06,
      ...
      0.18075E+14 } ,
    /*         --      7646 */
    { 0.11735E+06, 0.21659E+06, 0.35745E+06,
...
__host__ __device__
#endif
void QT_NOp ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --        46 */
    { 0.63956E+02, 0.90185E+02, 0.11642E+03,
...
__host__ __device__
#endif
void QT_HOBr ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[2][NT] = {
    /*         --       169 */
    { 0.24445E+04, 0.41206E+04, 0.60683E+04,
      ...
      0.10208E+08 } ,
    /*         --       161 */
    { 0.24350E+04, 0.41047E+04, 0.60448E+04,
...
__host__ __device__
#endif
void QT_C2H4 ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[2][NT] = {
    /*         --       221 */
    { 0.95843E+03, 0.16137E+04, 0.23744E+04,
      ...
      0.10885E+10 } ,
    /*         --       231 */
    { 0.39228E+04, 0.66051E+04, 0.97190E+04,
...
__host__ __device__
#endif
void QT_CH3OH ( const REAL_t  __attribute__((unused)) T,   /*  temperature in K, unused */
                const int8_t  __attribute__((unused)) iso, /*  isotope code (HITRANINDEX), unused */
                float* const gsi,       /*  state independent nuclear degeneracyfactor */
                REAL_t* const Qt)       /*  Total Internal Partition Function */
...
__host__ __device__
#endif
void QT_CH3Br ( const REAL_t T,   /*  temperature in K */
                const int8_t iso, /*  isotope code (HITRANINDEX) */
                float* const gsi,       /*  state independent nuclear degeneracyfactor */
                REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[2][NT] = {
    /*         --       219 */
    { 0.70299E+04, 0.11847E+05, 0.17442E+05,
      ...
      0.14536E+10 } ,
    /*         --       211 */
    { 0.70566E+04, 0.11892E+05, 0.17508E+05,
...
__host__ __device__
#endif
void QT_CH3CN ( const REAL_t T,   /*  temperature in K */
                const int8_t iso, /*  isotope code (HITRANINDEX) */
                float* const gsi,       /*  state independent nuclear degeneracyfactor */
                REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[4] = {3. , 6. , 6. , 12. } ;
  /*     CH3CN */
  const REAL_t QofT[4][NT] = {
    /*         --      2124 */
    { 0.54361E+04, 0.91953E+04, 0.13708E+05,
      ...
      0.39661E+11 } ,
    /*         --      2134 */
    { 0.10906E+05, 0.18458E+05, 0.27552E+05,
      ...
      0.84856E+11 } ,
    /*         --      3124 */
    { 0.11223E+05, 0.18985E+05, 0.28307E+05,
      ...
      0.85454E+11 } ,
    /*         --      3134 */
    { 0.22522E+05, 0.38117E+05, 0.56899E+05,
...
__host__ __device__
#endif
void QT_CF4 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --        29 */
    { 0.76233E+04, 0.12867E+05, 0.19059E+05,
...
__host__ __device__
#endif
void QT_C4H2 ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --      1221 */
    { 0.57628E+03, 0.84874E+03, 0.11789E+04,
...
__host__ __device__
#endif
void QT_HC3N ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[6] = {6. , 12. , 12. , 12. , 4. , 9. } ;
  /*      HC3N */
  const REAL_t QofT[6][NT] = {
    /*         --     12224 */
    { 0.16683E+04, 0.24538E+04, 0.33995E+04,
      ...
      0.22353E+11 } ,
    /*         --     12234 */
    { 0.33507E+04, 0.49290E+04, 0.68293E+04,
      ...
      0.55220E+11 } ,
    /*         --     12324 */
    { 0.33506E+04, 0.49280E+04, 0.68267E+04,
      ...
      0.38413E+11 } ,
    /*         --     13224 */
    { 0.34439E+04, 0.50672E+04, 0.70230E+04,
      ...
      0.83478E+11 } ,
    /*         --     12225 */
    { 0.11455E+04, 0.16850E+04, 0.23345E+04,
      ...
      0.16964E+11 } ,
    /*         --     22224 */
    { 0.27029E+04, 0.39999E+04, 0.55894E+04,
...
__host__ __device__
#endif
void QT_C2N2 ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[2][NT] = {
    /*         --      4224 */
    { 0.12049E+04, 0.17611E+04, 0.24195E+04,
      ...
      0.81994E+09 } ,
    /*         --      5225 */
    { 0.56948E+03, 0.83237E+03, 0.11436E+04,
...
__host__ __device__
#endif
void QT_CS ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[4] = {1. , 1. , 2. , 4. } ;
  /*        CS */
  const REAL_t QofT[4][NT] = {
    /*         --        22 */
    { 0.51416E+02, 0.72723E+02, 0.94044E+02,
      ...
      0.58380E+04 } ,
    /*         --        24 */
    { 0.52247E+02, 0.73900E+02, 0.95568E+02,
      ...
      0.59676E+04 } ,
    /*         --        32 */
    { 0.10889E+03, 0.15403E+03, 0.19920E+03,
      ...
      0.12630E+05 } ,
    /*         --        23 */
    { 0.20737E+03, 0.29330E+03, 0.37930E+03,
...
__host__ __device__
#endif
void QT_H2 ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[2] = {1. , 6. } ;
  /*        H2 */
  const REAL_t QofT[2][NT] = {
    /*         --        11 */
    { 0.15265E+01, 0.22243E+01, 0.29619E+01,
      ...
      0.86048E+02 } ,
    /*         --        12 */
    { 0.81692E+01, 0.10308E+02, 0.12557E+02,
...
__host__ __device__
#endif
void QT_SO ( const REAL_t T,   /*  temperature in K */
             const int8_t iso, /*  isotope code (HITRANINDEX) */
             float* const gsi,       /*  state independent nuclear degeneracyfactor */
             REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[3] = {1. , 1. , 1. } ;
  /*        SO */
  const REAL_t QofT[3][NT] = {
    /*         --        26 */
    { 0.15524E+03, 0.22725E+03, 0.29955E+03,
      ...
      0.20971E+05 } ,
    /*         --        46 */
    { 0.15819E+03, 0.23164E+03, 0.30538E+03,
      ...
      0.21545E+05 } ,
    /*         --        28 */
    { 0.16709E+03, 0.24485E+03, 0.32292E+03,
...
__host__ __device__
#endif
void QT_C3H4 ( const REAL_t T,/*  temperature in K */
               const int8_t iso, /*  isotope code (HITRANINDEX) */
               float* const gsi,       /*  state independent nuclear degeneracyfactor */
               REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --      1221 */
    { 0.38902E+04, 0.65995E+04, 0.99055E+04,
...
__host__ __device__
#endif
void QT_CH3 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t QofT[1][NT] = {
    /*         --      2111 */
    { 0.54079E+02, 0.92840E+02, 0.13834E+03,
...
__host__ __device__
#endif
void QT_CS2 ( const REAL_t T,   /*  temperature in K */
              const int8_t iso, /*  isotope code (HITRANINDEX) */
              float* const gsi,       /*  state independent nuclear degeneracyfactor */
              REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  ...
  const REAL_t xgj[4] = {1. , 1. , 4. , 2. } ;
  /* CS2 */
  const REAL_t QofT[4][NT] = {
    /*         --       222 */
    { 0.19143E+03, 0.27183E+03, 0.35501E+03,
      ...
      0.23738E+07 } ,
    /*         --       224 */
    { 0.39454E+03, 0.56027E+03, 0.73180E+03,
      ...
      0.50135E+07 } ,
    /*         --       223 */
    { 0.15549E+04, 0.22080E+04, 0.28838E+04,
      ...
      0.19517E+08 } ,
    /*         --       232 */
    { 0.38281E+03, 0.54376E+03, 0.71076E+03,
...
/* QT is just a mapping into the individual molecules */
/* It is written this way to be a near identical port of TIPS */
/* Ideally it can be refactored with the data simply mapped into an appropriately structured array */
/* and this section depricated */
#ifdef __NVCC__
__host__ __device__
#endif
void QT(const uint8_t molNum, /*  HITRAN molecule ID number */
        const REAL_t T,   /*  temperature in K */
        const uint8_t iso_OneBasedIdx,    /*  isotope code (HITRAN INDEX) */
        float* const gsi,      /*  state independent nuclear degeneracyfactor */
        REAL_t* const Qt)       /*  Total Internal Partition Function */

{
  /* correct iso from HITRAN index to Zero based array */
  const int8_t iso = iso_OneBasedIdx - 1;  /* HITRAN lists 10th most abundant iso (of CO2) as 0, so is negative one after correction, this is further corrected in QT_CO2*/
  switch (molNum)
  {
    case 1: /* H20 */
      QT_H2O(T,iso,gsi,Qt);
    ...
    case 19:
      ...
      break;
    case 20:
      QT_H2CO(T,iso,gsi,Qt);
      break;
    case 21:
      QT_HOCl(T,iso,gsi,Qt);
      break;
    case 22:
      QT_N2(T,iso,gsi,Qt);
      break;
    case 23:
      ...
      break;
    case 24:
      QT_CH3Cl(T,iso,gsi,Qt);
      break;
    case 25:
      QT_H2O2(T,iso,gsi,Qt);
      break;
    case 26:
      QT_C2H2(T,iso,gsi,Qt);
      break;
    case 27:
      QT_C2H6(T,iso,gsi,Qt);
      break;
    case 28:
      QT_PH3(T,iso,gsi,Qt);
      break;
    case 29:
      QT_COF2(T,iso,gsi,Qt);
      break;
    case 30:
      QT_SF6(T,iso,gsi,Qt);
      break;
    ...
    case 34:
      ...
      *Qt = 0.;
      break;
    case 35:
      QT_ClONO2(T,iso,gsi,Qt);
      break;
    case 36:
      QT_NOp(T,iso,gsi,Qt);
      break;
    case 37:
      QT_HOBr(T,iso,gsi,Qt);
      break;
    case 38:
      QT_C2H4(T,iso,gsi,Qt);
      break;
    case 39:
      *gsi=0.;
      *Qt = 0.;
      break;
    case 40:
      QT_CH3Br(T,iso,gsi,Qt);
      break;
    case 41:
      QT_CH3CN(T,iso,gsi,Qt);
      break;
    case 42:
      QT_CF4(T,iso,gsi,Qt);
      break;
    case 43:
      QT_C4H2(T,iso,gsi,Qt);
      break;
    case 44:
      QT_HC3N(T,iso,gsi,Qt);
      break;
    case 45:
      QT_C2N2(T,iso,gsi,Qt);
      break;
    case 46:
      QT_CS(T,iso,gsi,Qt);
      break;
    ...
    case 51:
      ...
      break;
    default:
      printf("Molecule Number %d not found. If a valid HITRAN number it has not been implimented here yet.\n", molNum);
      /* assert(false); */ /* only works on certain hardware...*/
  }
  return;
}
...
int testTips(int mol, int iso, REAL_t t){
...
int main(int argc , char* argv[]){
  ...
  if (argc == 4){
    /* get mol */
    int mol = (REAL_t)atoi(argv[1]);
    /* get iso */
    int iso = (REAL_t)atoi(argv[2]);
    /* get temp */
    REAL_t t = (REAL_t)strtod(argv[3],NULL);

    return testTips(mol,iso,t);

  }
  else{
    printf("Usage %s moleculeID isotopeID temp \n",argv[0]);
    return EXIT_FAILURE;
  }
}
