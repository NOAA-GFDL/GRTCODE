...
int main(int argc, char **argv)
{
    ...
    {
        /*Construt the path name for and open the specific file segment.*/
        char path[512];
        snprintf(path, 512, "%s/%s.%d", directory, basename, i);
        fprintf(stderr, "Opening dataset: %s\n", path);
        int ncid_segment;
        ...
        int num_variables;
        nc_catch(ncid_segment, &num_variables);
        ...
        for (j=0; j<num_variables; j++)
        {
            ...
            char name[128];
            memset(name, 0, 128*sizeof(char));
            nc_catch(nc_inq_varname(ncid_segment, j, name));
            fprintf(stderr, "Found variable %s name.\n");
            ...
            if (i == 0)
            {
                ...
                nc_type type;
                nc_catch(nc_inq_vartype(ncid_segment, j, &type));

                /*Get the units for the variable.*/
                char units[128];
                memset(units, 0, 128*sizeof(char));
                nc_catch(nc_get_att(ncid_segment, j, "units", units));

                /*Add the variable to the output dataset.*/
                nc_catch(nc_def_var(ncid, name, type, 0, NULL, &varid));
                nc_catch(nc_put_att_text(ncid, varid, "units", 128, units));
            }
            else
            {
            ...
            /*Get the data size and allocate buffers.*/
            int num_dims;
            int dimids[16];
            nc_catch(nc_inq_var(ncid, varid, NULL, NULL, &num_dims, dimids, NULL));
            size_t num_elements = 1;
            int k;
            for (k=0; k<num_dims; ++k)
            {
                size_t dim_size;
                nc_catch(nc_inq_dimlen(ncid, dimids[k], &dim_size));
                num_elements *= dim_size;
            }
            double *data = (double *)malloc(sizeof(double)*num_elements);
            double *data_rank = (double *)malloc(sizeof(double)*num_elements);

            /*Read in the data.*/
            nc_catch(nc_get_var_double(ncid, varid, data));
            nc_catch(nc_get_var_double(ncid_rank, varid_rank, data_rank));
...
#pragma omp parallel for default(shared) private(k)
            ...
            nc_catch(nc_put_var_double(ncid, varid, data));
